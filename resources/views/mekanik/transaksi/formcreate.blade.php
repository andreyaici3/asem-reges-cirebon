<div class="card-body">
                ...
                @foreach ($carts as $cart)
                    ...
                    <tr>
                        ...
                        <td>
                            @if ($cart['qty'] > 1)
                                <form
                                    action="{{ route('mekanik.transaksi.bypelanggan.min', ['id_pelanggan' => $pelanggan, 'code' => $cart['code']]) }}"
                            ...
                            @endif
                            <form
                                action="{{ route('mekanik.transaksi.bypelanggan.trash', ['id_pelanggan' => $pelanggan, 'code' => $cart['code']]) }}"
                                style="display: inline-block;" method="POST">
                                @csrf
                                @method('delete')
                            ...
                            <form
                                action="{{ route('mekanik.transaksi.bypelanggan.plus', ['id_pelanggan' => $pelanggan, 'code' => $cart['code']]) }}"
                                style="display: inline-block;" method="POST">
                                @csrf
                                @method('put')
    ...
    <div class="form-group">
        <form action="{{ route('mekanik.transaksi.bypelanggan.search', ['id_pelanggan' => $pelanggan]) }}"
            method="post">
            @csrf
    ...
    <div class="form-group">
        <label for="estimasi_jasa_pasang">Jasa Service</label>
        ...
        <div class="col-lg-6 mt-4 col-sm-12">
            ...
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tr>
                        <th>Total:</th>
                        <td><span style="font-weight: bold;" class="totbyr">Rp. 0</span></td>
                    </tr>
                </table>
            </div>
        </div>
    ...
    <script>
        $(function() {
            ...
            $("select[name=product]").change(function() {
                var code = $(this).val();
                var url = '{{ route('api.getproduk', ['code' => ':codes']) }}',
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        var action =
                            '{{ route('mekanik.transaksi.bypelanggan.add', ['code' => ':codes', 'id_pelanggan' => $pelanggan]) }}'
            ...
            $(".btn-simpan").click(function() {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        var action =
                            '{{ route('mekanik.transaksi.bypelanggan.store', ['id_pelanggan' => $pelanggan]) }}';
